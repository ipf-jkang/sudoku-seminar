# 행, 열, 박스에 n이 들어갈 수 있는지 체크하는 함수
# n이 현재 칸에 들어갈 수 있으면 True를 들어갈 수 없는 경우 False를 리턴함
def is_valid(grid, row, col, n):
    ...
    box_nums = [grid[r][s] for r in range(box_row_start, box_row_start + 3) for s in range(box_col_start, box_col_start + 3)]
...
def back_track(grid, row, col):
    ...
    for n in range(1, 10):
        if is_valid(grid, row, col, n):
            grid[row][col] = n
            if back_track(grid, row, col + 1): return True
...
def solve_sudoku(grid, row, col):
    ...
    for n in range(1, 10):
        if is_valid(grid, row, col, n):
            grid[row][col] = n
            if back_track(grid, row, col + 1): break
